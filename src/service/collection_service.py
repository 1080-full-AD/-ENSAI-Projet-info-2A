...
class CollectionVirtuelleService:
    ...
    def creer(
        ...
        if not all(isinstance(i, Manga) for i in liste_manga):
            raise TypeError(
                "Les collections virtuelles ne conteniennent que des mangas virtuelles :/"
            )
        ...
        if (
            CollectionDao().titre_existant(titre=titre, id_utilisateur=id_utilisateur)
            == True
        ):
        ...
        if CollectionDao().creer(collection=nouvelle_collection) and all(
            CollectionDao().ajouter_manga(collection=nouvelle_collection, manga=manga)
    ...
    def modifier_titre(self, collection, new_titre):
        ...
        if (
            CollectionDao().titre_existant(
                titre=new_titre, id_utilisateur=collection.id_utilisateur
            )
            == True
        ):
            raise ValueError("Vous avez déja une collection avec ce titre :/")
    ...
    def ajouter_manga(self, collection, new_manga):
        ...
        if isinstance(new_manga, MangaPhysique):
            raise ValueError(
                "les collections virtuelle ne contiennent que des mangas virtuelles"
            )
        if new_manga in CollectionDao().liste_manga(
            id_utilisateur=collection.id_utilisateur, titre_collec=collection.titre
        ):
